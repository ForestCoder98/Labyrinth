...
class NPC {
    constructor(row, col, type) {
        ...
        this.col = col;
        this.type = type;
        this.hitpoints = type === 'B' ? 3 : 1;
        this.strength = type === 'B' ? 5 : 3;
    }
    ...
    die() {
        ...
        eventText = "NPC ${this.type} defeated!";
    }
}
...
function oscillate(min, max) {
  ...
  class Labyrinth {
    ...
    update() {
        ...
        if (level[tRow] && level[tRow][tcol] === CONST.TELEPORT) {
            const currentTeleport = this.teleports.find(t => t.row === tRow && t.col === tcol);
            if (currentTeleport) {
                const otherTeleport = this.teleports.find(t => t.id !== currentTeleport.id);
                if (otherTeleport) {
